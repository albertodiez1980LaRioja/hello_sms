...
public class exportSprite
{
    ...
    private static void exportPallete(){
        try{
            ...
            for (String element :files){
                if(element.charAt(element.length()-1)=='p' && element.charAt(element.length()-2)=='m' && element.charAt(element.length()-3)=='b'){
    ...
    private static void exportFilePallete(String filename){
        ...
        //img= new BufferedImage(original.getWidth(), original.getHeight(), BufferedImage.TYPE_3BYTE_BGR);
        ...
        try{  
            ...
            for(int i=0;i<indexColor;i++){
                ...
                if (i!=0)
                    myWriterHex.write(",");
                myWriterHex.write("0x"+colorSMS(red, green,blue));
            }
            myWriter.close();
            myWriterHex.close();
    ...
    private static void exportSpriteFile(String filename, int colors[]){
        ...
        try{
            FileWriter myWriter2 = new FileWriter("sprite.inc");
            height = height / 16;
            width = width / 8;
            int tileIndex = 0;
            for(int h=0; h<height; h++){
                for(int w=0; w<width; w++){
                    ...
                    for(int ih=(h*16); ih<((h+1)*16); ih++){
                        for(int iw=(w*8); iw<((w+1)*8); iw++){
                            if(iw % 2 == 0)
                                myWriter2.write(" $");
                            rgb = img.getRGB(iw, ih);
                            red = normalizeColor((rgb >> 16 ) & 0x000000FF);
                            green = normalizeColor((rgb >> 8 ) & 0x000000FF);
                            blue = normalizeColor((rgb) & 0x000000FF);
    ...
    public static void main(String[] args) {
        ...
        if (args.length >= 2){
            // hay que convertirlo a una paleta lo mas cercana posible a una marter system, va de 85 en 85: 0,85,170,255. En el bmp de test no salen exactos
