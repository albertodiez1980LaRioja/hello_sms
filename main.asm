...
;./lib/entities.c:34: }
	...
	pop	ix
	ret
;./alex.c:18: void moveAlexOLD(int keys)
;	---------------------------------
; Function moveAlexOLD
; ---------------------------------
_moveAlexOLD::
	push	ix
	ld	ix,#0
	...
	ld	-2 (ix), l
	ld	-1 (ix), h
;./alex.c:21: if (keys & PORT_A_KEY_LEFT)
	ld	a, -2 (ix)
	and	a, #0x04
	ld	-6 (ix), a
	ld	-5 (ix), #0x00
;./alex.c:23: if (keys & PORT_A_KEY_RIGHT)
	ld	a, -2 (ix)
	and	a, #0x08
	ld	-4 (ix), a
	ld	-3 (ix), #0x00
;./alex.c:20: if ((keys & PORT_A_KEY_DOWN) && alex.state == EN_SUELO){
	bit	1, -2 (ix)
	jr	Z, 00109$
	ld	a, (#_alex + 5)
	sub	a, #0x02
	jr	NZ, 00109$
;./alex.c:22: alex.oriented = 1;
	ld	hl, #_alex + 4
;./alex.c:21: if (keys & PORT_A_KEY_LEFT)
	xor	a, a
	or	a, -6 (ix)
...
;./alex.c:23: if (keys & PORT_A_KEY_RIGHT)
	...
	or	a, -4 (ix)
	jr	Z, 00104$
;./alex.c:24: alex.oriented = 0;
	ld	(hl), #0x00
00104$:
;./alex.c:25: if (!alex.oriented)
	ld	a, (hl)
;./alex.c:26: alex.frame = 7;
;./alex.c:25: if (!alex.oriented)
	ld	-3 (ix), a
	or	a, a
	jr	NZ, 00106$
;./alex.c:26: alex.frame = 7;
	...
	jr	00107$
00106$:
;./alex.c:28: alex.frame = 15;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x0f
00107$:
;./alex.c:29: alex.lastChangeFrame = 15;
	ld	hl, #_alex + 3
	ld	(hl), #0x0f
;./alex.c:30: return; // not move
	jp	00180$
00109$:
;./alex.c:34: if (alex.state == PUÑETAZO) {
	ld	hl, #(_alex + 5)
	ld	c, (hl)
;./alex.c:35: alex.lastChangeFrame++;
;./alex.c:39: if (!alex.oriented)
;./alex.c:40: alex.frame = 0;
;./alex.c:34: if (alex.state == PUÑETAZO) {
	ld	a, c
	sub	a, #0x08
	jr	NZ, 00118$
;./alex.c:35: alex.lastChangeFrame++;
	...
	inc	a
	ld	(#(_alex + 3)),a
;./alex.c:36: if (alex.lastChangeFrame % 16 == 0)
	and	a, #0x0f
	jp	NZ,00180$
;./alex.c:38: alex.state = EN_SUELO;
	ld	hl, #(_alex + 5)
	ld	(hl), #0x02
;./alex.c:39: if (!alex.oriented)
	ld	a, (#(_alex + 4) + 0)
...
;./alex.c:40: alex.frame = 0;
	...
	ld	(hl), #0x00
00112$:
;./alex.c:41: if (alex.oriented)
	ld	a, (#(_alex + 4) + 0)
	or	a, a
	jp	Z,00180$
;./alex.c:42: alex.frame = 8;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x08
;./alex.c:44: return;
	jp	00180$
00118$:
;./alex.c:47: if (alex.state == PUÑETAZO_SALTANDO) {
	ld	a, c
	sub	a, #0x10
	jr	NZ, 00125$
;./alex.c:48: alex.lastChangeFrame++;
	ld	a, (#(_alex + 3) + 0)
	inc	a
	ld	(#(_alex + 3)),a
;./alex.c:49: if (alex.lastChangeFrame % 16 == 0)
	and	a, #0x0f
	jp	NZ,00180$
;./alex.c:51: alex.state = SALTANDO;
	ld	hl, #(_alex + 5)
	ld	(hl), #0x04
;./alex.c:52: if (!alex.oriented)
	ld	a, (#(_alex + 4) + 0)
	or	a, a
	jr	NZ, 00120$
;./alex.c:53: alex.frame = 5;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x05
	jp	00180$
00120$:
;./alex.c:55: alex.frame = 13; 
	ld	hl, #(_alex + 2)
	ld	(hl), #0x0d
;./alex.c:57: return;
	jp	00180$
00125$:
;./alex.c:61: if ((keys & PORT_A_KEY_LEFT) && alex.x > 8 )
	xor	a, a
	or	a, -6 (ix)
	jr	Z, 00136$
	ld	hl, #_alex
	...
	sub	a, c
	jr	NC, 00136$
;./alex.c:63: player_v_x = -1;
	ld	-3 (ix), #0xff
;./alex.c:64: alex.oriented = 1;
	ld	hl, #(_alex + 4)
	ld	(hl), #0x01
;./alex.c:65: if (alex.frame < 8 || alex.frame > 11)
	ld	hl, #(_alex + 2)
	ld	c, (hl)
	ld	a, c
	sub	a, #0x08
	...
	jr	NC, 00137$
00126$:
;./alex.c:66: alex.frame = 8;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x08
	jr	00137$
00136$:
;./alex.c:68: else if ((keys & PORT_A_KEY_RIGHT) && alex.x <240)
	xor	a, a
	or	a, -4 (ix)
	jr	Z, 00132$
	ld	a, (#_alex + 0)
	sub	a, #0xf0
	jr	NC, 00132$
;./alex.c:70: player_v_x = 1;
	ld	-3 (ix), #0x01
;./alex.c:71: alex.oriented = 0;
	ld	hl, #(_alex + 4)
	ld	(hl), #0x00
	jr	00137$
00132$:
;./alex.c:74: player_v_x = 0;
	ld	-3 (ix), #0x00
;./alex.c:75: alex.frame = 4;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x04
;./alex.c:76: if (alex.oriented)
	ld	a, (#(_alex + 4) + 0)
	or	a, a
	jr	Z, 00130$
;./alex.c:77: alex.frame = 12;
	ld	(hl), #0x0c
00130$:
;./alex.c:78: alex.lastChangeFrame = 15;
	ld	hl, #(_alex + 3)
	ld	(hl), #0x0f
00137$:
;./alex.c:80: alex.x = alex.x + player_v_x;
	ld	a, (#_alex + 0)
	ld	c, -3 (ix)
	add	a, c
	ld	(#_alex),a
;./alex.c:81: if (player_v_x != 0){
	ld	a, -3 (ix)
	or	a, a
	jr	Z, 00148$
;./alex.c:82: alex.lastChangeFrame++;
	ld	a, (#(_alex + 3) + 0)
	ld	-3 (ix), a
	inc	-3 (ix)
	ld	hl, #(_alex + 3)
	ld	a, -3 (ix)
	ld	(hl), a
;./alex.c:83: if (alex.lastChangeFrame % 16 == 0)
	ld	a, -3 (ix)
	and	a, #0x0f
	jr	NZ, 00148$
;./alex.c:85: alex.frame++;
	ld	hl, #(_alex + 2)
	ld	c, (hl)
	inc	c
	ld	hl, #(_alex + 2)
	ld	(hl), c
;./alex.c:86: if (alex.frame > 3 && !alex.oriented)
	ld	a, #0x03
	sub	a, c
	jr	NC, 00140$
	ld	a, (#(_alex + 4) + 0)
;./alex.c:87: alex.frame = 0;
	or	a,a
	jr	NZ, 00140$
	ld	(hl),a
00140$:
;./alex.c:88: if (alex.frame > 11 && alex.oriented)
	ld	hl, #(_alex + 2)
	ld	c, (hl)
	ld	a, #0x0b
	sub	a, c
	...
	or	a, a
	jr	Z, 00148$
;./alex.c:89: alex.frame = 8;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x08
00148$:
;./alex.c:95: finSalto = alex.y - 70;
;./alex.c:93: if ((keys & PORT_A_KEY_1) && alex.state == EN_SUELO) {
	bit	4, -2 (ix)
	jr	Z, 00150$
	ld	a, (#(_alex + 5) + 0)
	sub	a, #0x02
	jr	NZ, 00150$
;./alex.c:94: alex.state = SALTANDO;
	ld	hl, #(_alex + 5)
	ld	(hl), #0x04
;./alex.c:95: finSalto = alex.y - 70;
	ld	a, (#(_alex + 1) + 0)
...
;./alex.c:34: if (alex.state == PUÑETAZO) {
	ld	a, (#(_alex + 5) + 0)
;./alex.c:98: if (alex.state == SALTANDO || alex.state == PUÑETAZO_SALTANDO) {
	ld	-4 (ix), a
	sub	a, #0x04
	ld	a, #0x01
	jr	Z, 00472$
...
00472$:
	...
	jr	NZ, 00166$
00165$:
;./alex.c:99: if(alex.state == SALTANDO) {
	ld	a, -3 (ix)
	or	a, a
	jr	Z, 00156$
;./alex.c:100: if (!alex.oriented)
	...
	or	a, a
	jr	NZ, 00153$
;./alex.c:101: alex.frame = 6;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x06
	jr	00156$
00153$:
;./alex.c:103: alex.frame = 14;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x0e
00156$:
;./alex.c:105: if(alex.y == EN_SUELO || alex.y == 1 || alex.y < finSalto) {
	ld	hl, #(_alex + 1)
	ld	e, (hl)
	ld	a,e
	cp	a,#0x02
...
;./alex.c:106: vY = 2;
	...
	ld	(_vY), hl
00158$:
;./alex.c:108: if ((alex.y == 150 || alex.y == 151) && vY > 0) {
	ld	a, (#(_alex + 1) + 0)
	cp	a, #0x96
	jr	Z, 00164$
	sub	a, #0x97
...
00481$:
	jp	P, 00166$
;./alex.c:109: alex.state = EN_SUELO;
	ld	hl, #(_alex + 5)
	ld	(hl), #0x02
;./alex.c:110: vY = 0;
	ld	hl, #0x0000
	ld	(_vY), hl
;./alex.c:111: alex.frame = 0;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x00
00166$:
;./alex.c:114: alex.y += vY;
	ld	hl, #(_alex + 1)
	ld	c, (hl)
	ld	a, (_vY+0)
	ld	-5 (ix), a
	...
	ld	a, -3 (ix)
	ld	(hl), a
;./alex.c:115: if ((keys & PORT_A_KEY_2) && alex.state == EN_SUELO){
	ld	a, -2 (ix)
	and	a, #0x20
	ld	-4 (ix), a
	ld	-3 (ix), #0x00
	...
	sub	a, #0x02
	jr	NZ, 00172$
;./alex.c:116: alex.state = PUÑETAZO;
	ld	hl, #(_alex + 5)
	ld	(hl), #0x08
;./alex.c:117: if (!alex.oriented)
	ld	a, (#(_alex + 4) + 0)
	or	a, a
	jr	NZ, 00169$
;./alex.c:118: alex.frame = 5;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x05
	jr	00170$
00169$:
;./alex.c:120: alex.frame = 13; 
	ld	hl, #(_alex + 2)
	ld	(hl), #0x0d
00170$:
;./alex.c:121: alex.lastChangeFrame = 0;
	ld	hl, #(_alex + 3)
	ld	(hl), #0x00
00172$:
;./alex.c:123: if ((keys & PORT_A_KEY_2) && alex.state == SALTANDO){
	xor	a, a
	or	a, -4 (ix)
	jr	Z, 00180$
	ld	a, (#(_alex + 5) + 0)
	sub	a, #0x04
	jr	NZ, 00180$
;./alex.c:124: alex.state = PUÑETAZO_SALTANDO;
	ld	hl, #(_alex + 5)
	ld	(hl), #0x10
;./alex.c:125: if (!alex.oriented)
	ld	a, (#(_alex + 4) + 0)
	or	a, a
	jr	NZ, 00175$
;./alex.c:126: alex.frame = 5;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x05
	jr	00176$
00175$:
;./alex.c:128: alex.frame = 13; 
	ld	hl, #(_alex + 2)
	ld	(hl), #0x0d
00176$:
;./alex.c:129: alex.lastChangeFrame = 0;
	ld	hl, #(_alex + 3)
	ld	(hl), #0x00
00180$:
;./alex.c:132: }
	...
	pop	ix
	ret
;./alex.c:135: unsigned char canUp(){
;	---------------------------------
; Function canUp
; ---------------------------------
_canUp::
;./alex.c:136: if (alex.x < 2)
	ld	a, (#_alex + 0)
	sub	a, #0x02
	jr	NC, 00102$
;./alex.c:137: return 0;
...
;./alex.c:139: }
	ret
;./alex.c:141: unsigned char canDown() {
;	---------------------------------
; Function canDown
; ---------------------------------
_canDown::
;./alex.c:142: if (alex.y > 155)
	ld	hl, #_alex+1
	ld	c, (hl)
	ld	a, #0x9b
	sub	a, c
...
;./alex.c:145: }
	ret
;./alex.c:147: unsigned char canLeft() {
;	---------------------------------
; Function canLeft
; ---------------------------------
_canLeft::
;./alex.c:148: if (alex.x < 9)
	ld	a, (#_alex + 0)
	sub	a, #0x09
	jr	NC, 00102$
;./alex.c:149: return 0;
...
;./alex.c:151: }
	ret
;./alex.c:153: unsigned char canRight() {
;	---------------------------------
; Function canRight
; ---------------------------------
_canRight::
;./alex.c:154: if (alex.x > 238)
	ld	hl, #_alex+0
	ld	c, (hl)
	ld	a, #0xee
	sub	a, c
...
;./alex.c:157: }
	ret
;./alex.c:159: void moveAlexSuelo(int keys) {
;	---------------------------------
...
_moveAlexSuelo::
	...
	ld	-2 (ix), l
	ld	-1 (ix), h
;./alex.c:161: if (keys & PORT_A_KEY_LEFT)
	ld	a, -2 (ix)
	and	a, #0x04
	ld	-6 (ix), a
	ld	-5 (ix), #0x00
;./alex.c:163: if (keys & PORT_A_KEY_RIGHT)
	ld	a, -2 (ix)
	and	a, #0x08
	ld	-4 (ix), a
	ld	-3 (ix), #0x00
;./alex.c:160: if ((keys & PORT_A_KEY_DOWN)){
	bit	1, -2 (ix)
	jr	Z, 00109$
;./alex.c:161: if (keys & PORT_A_KEY_LEFT)
	xor	a, a
...
;./alex.c:163: if (keys & PORT_A_KEY_RIGHT)
	...
	or	a, -4 (ix)
	jr	Z, 00104$
;./alex.c:164: alex.oriented = 0;
	ld	hl, #_alex+4
	ld	(hl), #0x00
00104$:
;./alex.c:165: if (!alex.oriented)
	ld	a,(#_alex + 4)
;./alex.c:166: alex.frame = 7;
;./alex.c:165: if (!alex.oriented)
	ld	-3 (ix), a
	or	a, a
	jr	NZ, 00106$
;./alex.c:166: alex.frame = 7;
	...
	jr	00107$
00106$:
;./alex.c:168: alex.frame = 15;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x0f
00107$:
;./alex.c:169: alex.lastChangeFrame = 15;
	ld	hl, #_alex + 3
	ld	(hl), #0x0f
;./alex.c:170: return; // not move
	jp	00135$
00109$:
;./alex.c:173: if (keys & PORT_A_KEY_2) {
	bit	5, -2 (ix)
	jr	Z, 00114$
;./alex.c:174: if (alex.y > 100)
	ld	bc, #_alex+1
	ld	a, (bc)
	ld	e, a
	ld	a, #0x64
	sub	a, e
	jr	NC, 00111$
;./alex.c:175: maxSalto = alex.y - 100;
	ld	a, e
	ld	hl, #_maxSalto
	add	a, #0x9c
	ld	(hl), a
...
;./alex.c:179: return;
	jr	00135$
00114$:
;./alex.c:181: if ((keys & PORT_A_KEY_LEFT) && alex.x > 8 )
	xor	a, a
	or	a, -6 (ix)
	jr	Z, 00132$
	ld	hl, #_alex+0
...
;./alex.c:183: alex.x -= 1;
	dec	c
	ld	(hl), c
;./alex.c:184: alex.oriented = 1;
	ld	hl, #_alex + 4
	ld	(hl), #0x01
;./alex.c:185: alex.lastChangeFrame++;
	ld	hl, #_alex + 3
	inc	(hl)
	ld	a, (hl)
;./alex.c:188: alex.frame++;
	ld	de, #_alex + 2
;./alex.c:186: if (alex.lastChangeFrame == 11) {
;./alex.c:187: alex.lastChangeFrame = 0;
	sub	a, #0x0b
	jr	NZ, 00116$
	ld	(hl), a
;./alex.c:188: alex.frame++;
	...
	ld	(de), a
00116$:
;./alex.c:190: if (alex.frame > 11 || alex.frame < 8)
	ld	a, (de)
	ld	c, a
	ld	a, #0x0b
	sub	a, c
	...
	jr	NC, 00135$
00117$:
;./alex.c:191: alex.frame = 8;
	ld	a, #0x08
	ld	(de), a
	jr	00135$
00132$:
;./alex.c:193: else if ((keys & PORT_A_KEY_RIGHT) && alex.x <240)
	xor	a, a
	or	a, -4 (ix)
	jr	Z, 00128$
	ld	bc, #_alex+0
...
;./alex.c:195: alex.x += 1;
	inc	a
	ld	(bc), a
;./alex.c:196: alex.oriented = 0;
	ld	hl, #_alex + 4
	ld	(hl), #0x00
;./alex.c:197: alex.lastChangeFrame++;
	ld	hl, #_alex + 3
	inc	(hl)
	ld	a, (hl)
;./alex.c:200: alex.frame++;
	ld	de, #_alex + 2
;./alex.c:198: if (alex.lastChangeFrame == 11) {
;./alex.c:199: alex.lastChangeFrame = 0;
	sub	a, #0x0b
	jr	NZ, 00121$
	ld	(hl), a
;./alex.c:200: alex.frame++;
	...
	ld	(de), a
00121$:
;./alex.c:202: if (alex.frame > 3 || alex.frame < 0)
	ld	a, (de)
	ld	c, a
	ld	a, #0x03
	sub	a, c
	jr	NC, 00135$
;./alex.c:203: alex.frame = 0;
	xor	a, a
	ld	(de), a
	jr	00135$
00128$:
;./alex.c:207: alex.frame = 4;
	ld	hl, #(_alex + 2)
	ld	(hl), #0x04
;./alex.c:208: if (alex.oriented)
	ld	a, (#_alex + 4)
	or	a, a
	jr	Z, 00126$
;./alex.c:209: alex.frame = 12;
	ld	(hl), #0x0c
00126$:
;./alex.c:210: alex.lastChangeFrame = 10;
	ld	hl, #_alex + 3
	ld	(hl), #0x0a
00135$:
;./alex.c:212: }
	...
	pop	ix
	ret
;./alex.c:214: void moveAlexAire(int keys, unsigned char puedeSubir, unsigned char puedeDerecha, unsigned char puedeIzquieda) {
;	---------------------------------
...
_moveAlexAire::
	...
	add	ix,sp
	ex	de, hl
;./alex.c:215: if (keys & PORT_A_KEY_LEFT) {
	bit	2, e
	jr	Z, 00104$
;./alex.c:216: alex.oriented = 1;
	ld	bc, #_alex+0
	ld	hl, #(_alex + 4)
	ld	(hl), #0x01
;./alex.c:217: if (puedeIzquieda)
	ld	a, 6 (ix)
	or	a, a
	jr	Z, 00104$
;./alex.c:218: alex.x -= 2;
	...
	ld	(bc), a
00104$:
;./alex.c:220: if (keys & PORT_A_KEY_RIGHT) {
	bit	3, e
	jr	Z, 00108$
;./alex.c:221: alex.oriented = 0;
	ld	bc, #_alex+0
	ld	hl, #(_alex + 4)
	ld	(hl), #0x00
;./alex.c:222: if (puedeDerecha)
	ld	a, 5 (ix)
	or	a, a
	jr	Z, 00108$
;./alex.c:223: alex.x += 2;
	...
	ld	(bc), a
00108$:
;./alex.c:225: if (!alex.oriented)
	ld	hl, #_alex + 4
	ld	b, (hl)
;./alex.c:226: alex.frame = 6;
	ld	hl, #_alex + 2
;./alex.c:225: if (!alex.oriented)
	ld	a, b
	or	a, a
...
;./alex.c:226: alex.frame = 6;
	...
	jr	00111$
00110$:
;./alex.c:228: alex.frame = 14;
	ld	(hl), #0x0e
00111$:
;./alex.c:229: if (alex.y > maxSalto) {
	ld	bc, #_alex + 1
	ld	a, (bc)
	ld	d, a
;./alex.c:230: alex.y -= 2;
...
;./alex.c:230: alex.y -= 2;
	...
	jr	00115$
00113$:
;./alex.c:233: alex.y += 2; // bajando
	ld	a, e
	inc	a
	inc	a
	ld	(bc), a
;./alex.c:234: maxSalto = 255;
	ld	iy, #_maxSalto
	ld	0 (iy), #0xff
00115$:
;./alex.c:236: }
	...
	inc	sp
	jp	(hl)
;./alex.c:239: void moveAlex(int keys) {
;	---------------------------------
; Function moveAlex
; ---------------------------------
_moveAlex::
	...
	add	ix,sp
	dec	sp
;./alex.c:240: unsigned char puedeBajar = canDown();
	push	hl
	call	_canDown
	ld	c, a
	push	bc
	...
	pop	bc
	pop	hl
;./alex.c:244: if (puedeBajar)
	inc	c
	dec	c
	jr	Z, 00102$
;./alex.c:245: moveAlexAire(keys, puedeSubir, puedeDerecha, puedeIzquierda);
	ld	d,a
	push	de
	ld	a, -1 (ix)
	push	af
